...
class Major:
    ...
    def add_course(self, course, r_e):
        if r_e == "R":
...
class Student:
    ...
    def add_course(self, course, grades):
        if grades in ["A", "A-", "B+", "B", "B-", "C"]:
            self.courses[course] = grades


    def details(self):
        return [self.s_cwid, self.s_name, self.major, sorted(self.courses.keys()), self.courses, self.r_e]
